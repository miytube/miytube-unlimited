...
const PrivateVideos = () => {
  ...
  // Private videos - filter by isPrivate flag or private tag
  const privateVideos = [...uploadedVideos]
    .filter(video => 
      video.category?.toLowerCase() !== 'shorts' &&
      (video.tags?.some(t => t.toLowerCase() === 'private') || video.category?.toLowerCase() === 'private')
    )
    ...
    <Layout>
      <div className="py-6 animate-fade-in w-full max-w-[1400px] mx-auto px-2 sm:px-4">
        <p className="text-sm text-muted-foreground mb-2">
          <span className="font-semibold text-primary">MiyTube</span> / Video Library / Private
        </p>
        
        <div className="flex items-center gap-3 mb-8">
          <Lock className="h-8 w-8 text-primary" />
          <h1 className="text-3xl font-bold">Private Videos</h1>
          <p className="text-muted-foreground ml-2">Your private video collection</p>
        ...
        {privateVideos.length > 0 ? (
          ...
          <div className="p-6 bg-muted/20 rounded-lg text-center">
            <p className="text-muted-foreground mb-4">No private videos yet. Tag videos as "private" to see them here.</p>
