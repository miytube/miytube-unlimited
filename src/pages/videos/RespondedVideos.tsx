...
import { Reply, Upload } from 'lucide-react';
import { Link } from 'react-router-dom';
import { useUploadedVideos } from '@/context/UploadedVideosContext';

const RespondedVideos = () => {
  const { uploadedVideos } = useUploadedVideos();
  
  // Responded videos - filter by responded tag
  const respondedVideos = [...uploadedVideos]
    .filter(video => 
    ...
    <Layout>
      <div className="py-6 animate-fade-in w-full max-w-[1400px] mx-auto px-2 sm:px-4">
        <p className="text-sm text-muted-foreground mb-2">
          <span className="font-semibold text-primary">MiyTube</span> / Video Library / Responded
        </p>
        
        <div className="flex items-center gap-3 mb-8">
          <Reply className="h-8 w-8 text-primary" />
          <h1 className="text-3xl font-bold">Responded Videos</h1>
          <p className="text-muted-foreground ml-2">Videos with your responses</p>
        ...
        {respondedVideos.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {respondedVideos.map((video) => (
              <VideoCard
                key={video.id}
                id={video.id}
                title={video.title}
          ...
          <div className="p-6 bg-muted/20 rounded-lg text-center">
            <p className="text-muted-foreground mb-4">No responded videos yet.</p>
...
export default RespondedVideos;
